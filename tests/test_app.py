...
class QuizAppTestCase(unittest.TestCase):
    ...
    def _create_test_data(self):
        # Create test users
        users = [
            User(username='test_user', email='test@test.com', 
                 password_hash=generate_password_hash('password123')),
            User(username='another_user', email='another@test.com',
                 password_hash=generate_password_hash('password123'))
        ]
        for user in users:
            db.session.add(user)
        ...
        scores = [
            Score(user_id=1, quiz_name='Python Basics', score=8, max_score=10,
                 date_taken=base_date - timedelta(days=1)),
            Score(user_id=1, quiz_name='JavaScript Basics', score=7, max_score=10,
                 date_taken=base_date - timedelta(days=2)),
            Score(user_id=2, quiz_name='Python Basics', score=9, max_score=10,
                 date_taken=base_date - timedelta(days=1))
        ]
        for score in scores:
    ...
    def test_quiz_flow(self):
        ...
        response = self.client.post('/quiz', data={
            'quiz_type': 'Python Basics'
        }, follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        
        # Answer questions
        response = self.client.post('/question', data={
            'answer': '0'  # Assuming multiple choice with index 0
        })
        self.assertEqual(response.status_code, 200)
