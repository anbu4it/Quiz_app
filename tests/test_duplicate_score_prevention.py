...
def test_duplicate_score_prevention():
    ...
    with app.app_context():
        db.create_all()
        user = User(username='dup_user', email='dup@example.com', password_hash=generate_password_hash('password123'))
        db.session.add(user)
        db.session.commit()
    # Login
    client.post('/login', data={'username': 'dup_user', 'password': 'password123'}, follow_redirects=True)
    # Start quiz
